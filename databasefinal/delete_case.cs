    ...
    public partial class delete_case : Form
    {
        ...
        public void connectsql()
        {
            try
            {
                ...
                string selectQuery = "SELECT Ca_model FROM c_case WHERE 1";
                connection.Open();
        ...
        private void button_delete_Click(object sender, EventArgs e)
        {
            ...
            if (Result == DialogResult.OK)
            {
                ...
                {
                    MySqlConnection connection = new MySqlConnection("server=100.25.117.10;uid=admin;pwd=0000;database=pc;CharSet=utf8;");
                    string selectQuery = "DELETE FROM `c_case` WHERE Ca_model='" + comboBox_case.Text + "' ";
                    connection.Open();
                    MySqlCommand command = new MySqlCommand(selectQuery, connection);
        ...
        {
            string select = this.comboBox_case.Text;
            string connString = "server=100.25.117.10;uid=admin;pwd=0000;database=pc;CharSet=utf8;";
            using (MySqlConnection mcon = new MySqlConnection(connString))
            using (MySqlCommand cmd = mcon.CreateCommand())
            {
