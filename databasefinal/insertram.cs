...
{
    public partial class insertram : Form
    {
        public insertram()
        {
            InitializeComponent();
        ...
        private void insertram_Load(object sender, EventArgs e)
        {
        ...
        private void insert_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string selectQuery = "INSERT INTO `ram`(`R_model`, `R_capacity`, `propose`, `hz`, `brand`, `price`) VALUES ('" + R_model.Text + "','" + R_capacity.Text + "','" + propose.Text + "','" + hz.Text + "','" + brand.Text + "','" + price.Text + "') ";
                connection.Open();
        ...
        private void update_Click(object sender, EventArgs e)
        {
            ...
            {
                MySqlConnection connection = new MySqlConnection("server=100.25.117.10;uid=admin;pwd=0000;database=pc;CharSet=utf8;");
                string selectQuery = "UPDATE `ram` SET `R_model`='" + R_model.Text + "',`R_capacity`='" + R_capacity.Text + "',`propose`='" + propose.Text + "',`hz`='" + hz.Text + "',`brand`='" + brand.Text + "',`price`='" + price.Text + "' WHERE R_model='" + select.Text + "'";
                connection.Open();
                MySqlCommand command = new MySqlCommand(selectQuery, connection);
        ...
        public void display_select()
        {
            ...
            {
                MySqlConnection connection = new MySqlConnection("server=100.25.117.10;uid=admin;pwd=0000;database=pc;CharSet=utf8;");
                string selectQuery = "SELECT * FROM ram WHERE 1";
                connection.Open();
                MySqlCommand command = new MySqlCommand(selectQuery, connection);
                ...
                while (reader.Read())
                {
                    select.Items.Add(reader.GetString("R_model"));
                }
                connection.Close();
            }
            catch (Exception ex)
        ...
        public void display_hz()
        {
            try
            {
                ...
                while (reader.Read())
                {
                    hz.Items.Add(reader.GetString("hz"));
                }
                connection.Close();
            }
            catch (Exception ex)
        ...
        public void display_brand()
        {
            ...
            {
                MySqlConnection connection = new MySqlConnection("server=100.25.117.10;uid=admin;pwd=0000;database=pc;CharSet=utf8;");
                string selectQuery = "SELECT * FROM ram_brand WHERE 1";
                connection.Open();
                MySqlCommand command = new MySqlCommand(selectQuery, connection);
