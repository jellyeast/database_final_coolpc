...
{
    public partial class insert_storage_brand : Form
    {
        public insert_storage_brand()
        {
            InitializeComponent();
            display_select();
        }
        ...
        private void select_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            using (MySqlCommand cmd = mcon.CreateCommand())
            {
                ...
                cmd.CommandText = "SELECT * FROM `storage_brand` WHERE brand='" + select + "'";
                using (MySqlDataReader reader = cmd.ExecuteReader())
                {
                    reader.Read();
                    sb.Text = reader.GetString("brand");
                }
                mcon.Close();
            }
        }
        ...
        private void insert_Click(object sender, EventArgs e)
        {
            ...
            {
                MySqlConnection connection = new MySqlConnection("server=100.25.117.10;uid=admin;pwd=0000;database=pc;CharSet=utf8;");
                string selectQuery = "INSERT INTO `storage_brand`(`brand`) VALUES ('" + sb.Text + "')";
                connection.Open();
                MySqlCommand command = new MySqlCommand(selectQuery, connection);
        ...
        private void update_Click(object sender, EventArgs e)
        {
            ...
            {
                MySqlConnection connection = new MySqlConnection("server=100.25.117.10;uid=admin;pwd=0000;database=pc;CharSet=utf8;");
                string selectQuery = "UPDATE `storage_brand` SET `brand`='" + sb.Text + "' WHERE brand='" + select.Text + "'";
                connection.Open();
                MySqlCommand command = new MySqlCommand(selectQuery, connection);
