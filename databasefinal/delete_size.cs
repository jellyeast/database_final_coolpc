...
{
    public partial class delete_size : Form
    {
        public delete_size()
        {
            InitializeComponent();
        ...
        private void button_delete_Click(object sender, EventArgs e)
        {
            ...
            if (Result == DialogResult.OK)
            {
                try
                {
                    ...
                    string selectQuery = "DELETE FROM `size` WHERE size='" + comboBox_size.Text + "'";
                    connection.Open();
        ...
        public void connectsql()
        {
            ...
            {
                MySqlConnection connection = new MySqlConnection("server=100.25.117.10;uid=admin;pwd=0000;database=pc;CharSet=utf8;");
                string selectQuery = "SELECT size FROM size WHERE 1";
                connection.Open();
                MySqlCommand command = new MySqlCommand(selectQuery, connection);
                ...
                while (reader.Read())
                {
                    comboBox_size.Items.Add(reader.GetString("size"));
                }
                connection.Close();
            }
            catch (Exception ex)
